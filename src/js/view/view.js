...
const showLoading = () => {
    let loading = document.querySelector(".preload-content");
...
//init this materialize framework
const init =  () => {
...
const viewHtmlFavorite = async () => {
   hideLoading();
    ...
    let elmHtml = ``;
      ...
      if (elmHtml === ``) {
        elmHtml = `<h5 class='center black-text'> Anda belum memiliki Teams Favorite </h5>`
      }
      content.innerHTML = elmHtml;
    });
    ...
    await dbSquadFavorite.getAll().then((items)=>{
      ...
      if (elmHtmlFav === ``) {
        elmHtmlFav = `<h5 class='center black-text'> Anda belum memiliki Player Favorite </h5>`
        }
...
// view hmtl Eror if progress FetcApi
const viewHtmlErorApi = () => {
  hideLoading();
  let elmHtml  = document.querySelector(".body-content");
  elmHtml.innerHTML = `<h5 class='center blue-text'> Ups.. Maaf Ada yang tidak beres dari server
                        Silahkan Refresh kembali dan pastikan koneksi internet anda stabil terlebih dahulu </h5>`;
  mode();
}    
export { viewHtmlStandings , viewHtmlMatch , viewHtmlTeams , viewHtmlDetailTeam, mode , viewHtmlFavorite , setDarkMode , parseHttpToHttps , init , viewHtmlErorApi , showLoading};